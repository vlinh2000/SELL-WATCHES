...
module.exports = {
    get_yeuthichs: async (req, res) => {
        try {
            const sql = `SELECT * FROM YEU_THICH`;
    ...
    post_yeuthichs: async (req, res) => {
        try {
            const { USER_ID, MA_SP } = req.body;
            const MA_YEU_THICH = randomString();

            const sql = `INSERT INTO YEU_THICH(MA_YEU_THICH, USER_ID, MA_SP) 
                                        VALUES ('${MA_YEU_THICH}','${USER_ID}','${MA_SP}')`;
    ...
    delete_yeuthichs: async (req, res) => {
        try {
            ...
            const sql = `DELETE FROM YEU_THICH WHERE MA_YEU_THICH='${yeuthichID}'`;
            await executeQuery(sql);
